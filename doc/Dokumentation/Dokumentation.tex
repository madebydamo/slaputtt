%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Programming/Coding Assignment
% LaTeX Template
%
% This template has been downloaded from:
% http://www.latextemplates.com
%
% Original author:
% Ted Pavlic (http://www.tedpavlic.com)
%
% Note:
% The \lipsum[#] commands throughout this template generate dummy text
% to fill the template out. These commands should all be removed when 
% writing assignment content.
%
% This template uses a Perl script as an example snippet of code, most other
% languages are also usable. Configure them in the "CODE INCLUSION 
% CONFIGURATION" section.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%----------------------------------------------------------------------------------------
%	PACKAGES AND OTHER DOCUMENT CONFIGURATIONS
%----------------------------------------------------------------------------------------

\documentclass[11pt]{article}

\usepackage{fancyhdr} % Required for custom headers
\usepackage{lastpage} % Required to determine the last page for the footer
\usepackage{extramarks} % Required for headers and footers
\usepackage[usenames,dvipsnames]{xcolor} % Required for custom colors
\usepackage{graphicx} % Required to insert images
\usepackage{wrapfig} % Wraping text around images
\usepackage{listings} % Required for insertion of code
\usepackage{courier} % Required for the courier font
\usepackage{lipsum} % Used for inserting dummy 'Lorem ipsum' text into the template
\usepackage[utf8]{inputenc} % UTF8 for öäöüéè...
\usepackage[T1]{fontenc} % Wordsplitting
\usepackage[many]{tcolorbox} % Textbox
\usepackage{xargs} % For more then one optional Argument
%\usepackage{ngerman} % German Captures
\usepackage[autostyle=true,german=quotes]{csquotes} % Anführungszeichen nach Deutschem Norm
\usepackage[sort]{acro} %Fussnote mit Verzeichnis
\usepackage{pgfgantt}
\usepackage{rotating}
\usepackage{float}
\usepackage{titlesec}
\usepackage{hyperref}
\usepackage[useregional]{datetime2}
\usepackage{makecell}
\usepackage{tabularx}
\usepackage{csvsimple}
\usepackage{longtable}
\usepackage{uarial}
\usepackage{upquote}
\usepackage{enumitem}
\usepackage{nameref}
\usepackage{pgfplots}
\usepackage{subcaption}
\usepackage{url}
\usepackage{microtype}
\usepackage{multimedia}
\usepackage{array}
\usepackage{pgfplotstable}
\usepackage{eso-pic} % Background image
\usepackage{apacite}
\usepackage[left=3cm, right=2cm, top=2.5cm, bottom=2.5cm, paper=a4paper]{geometry}
\pgfplotsset{compat=1.16}
\renewcommand{\familydefault}{\sfdefault}

% Margins
%\topmargin=-0.45in
%\evensidemargin=0in
%\oddsidemargin=0in
%\textwidth=6.5in %6.5in
%\textheight=9.0in
%\headsep=0.25in

\linespread{1.25} % Line spacing
\setlist[1]{nolistsep}

%\renewcommand{\familydefault}{\sfdefault}

% Set up the header and footer
\pagestyle{fancy}
\lhead{Stefan Jampen \& Damian Moser} % Top left header
\chead{IDPA} % Top center head
\rhead{\nouppercase{\rightmark}} % Top right header \firstxmark
\lfoot{\href{https://slaputtt.firebaseapp.com/}{https://slaputtt.firebaseapp.com/}} % Bottom left footer
% Neues Logo suchen
\cfoot{}
% \cfoot{\vspace*{-1.88\baselineskip}\includegraphics[scale=0.33]{abacuslogo}\vspace*{1.88\baselineskip}} % Bottom center footer
\rfoot{Side\ \thepage\ of\ \protect\pageref{LastPage}} % Bottom right footer
\renewcommand\headrulewidth{0.4pt} % Size of the header rule
\renewcommand\footrulewidth{0.4pt} % Size of the footer rule

\setlength\parindent{0pt} % Removes all indentation from paragraphs
%\setcounter{tocdepth}{2}
%\setcounter{secnumdepth}{0} % Removes default section numbers
\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{4}

%----------------------------------------------------------------------------------------
%	CODE INCLUSION CONFIGURATION
%----------------------------------------------------------------------------------------




%----------------------------------------------------------------------------------------
%	DOCUMENT STRUCTURE COMMANDS
%	Skip this unless you know what you're doing
%----------------------------------------------------------------------------------------


%----------------------------------------------------------------------------------------
%	NAME AND CLASS SECTION
%----------------------------------------------------------------------------------------

\renewcommand{\Nameref}[1]{`\nameref{#1}' on page~\pageref{#1}}
\titleformat{\paragraph}
{\normalfont\normalsize\bfseries}{\theparagraph}{1em}{}
\titlespacing*{\paragraph}
{0pt}{3.25ex plus 1ex minus .2ex}{1.5ex plus .2ex}
%----------------------------------------------------------------------------------------
%	TITLE PAGE
%----------------------------------------------------------------------------------------
\newcommand\BackgroundPic{
\put(0,-175){%
\parbox[b][\textheight]{\textwidth}{%
\vfill
\centering
\includegraphics[width=\textwidth,height=\textheight,%
keepaspectratio]{background2.jpg}%
\vfill
}}}
\newcommand{\HRule}[1]{\rule{\linewidth}{#1}}
\title{ \normalsize \textsc{Documentation\\IDPA}
		\\ [2.0cm]
		\HRule{2pt} \\
		\LARGE \textbf{\uppercase{Target oriented application of a SLAP algorithm for Ultimate Tic Tac Toe}}
		\HRule{2pt} \\ [0.5cm]
		\normalsize  \vspace*{5\baselineskip}}

\date{}

\author{
		Stefan Jampen \\ 
        	Damian Moser \\
		Guided By - Alejandro González and Barbara Becher \\
		Gewerbliche Berufsschule St.Gallen}



%\title{
%\LARGE{Dokumentation}
%}
%
%%\author{\textbf{\hmwkAuthorName}}
%\author{}
%\date{} % Insert date here if you want it to appear below your name

%----------------------------------------------------------------------------------------

\graphicspath{{./img/}}
\newcommand\todo[1]{\textcolor{red}{\\\\//TODO #1\\}}

%\newcolumntype{L}[1]{>{\raggedright\let\newline\\\arraybackslash\hspace{0}}{#1}}
%\newcolumntype{C}[1]{>{\centering\let\newline\\\arraybackslash\hspace{0}}m{#1}}
%\newcolumntype{R}[1]{>{\raggedleft\let\newline\\\arraybackslash\hspace{.2in}}m{#1}}
\newcolumntype{R}{>{\raggedleft\arraybackslash}p{.2in}}


\abovecaptionskip=3pt % above caption
\belowcaptionskip=\baselineskip % below caption

\newfloat{fixedpic}{H}{fixedpic}
\newcommand{\captionof}[2]{\caption{#2}}

%\newenvironment{fixedpic}{
%\break
%\begin{minipage}{\linewidth}
%\abovecaptionskip=3pt % above caption
%\belowcaptionskip=\baselineskip % below caption
%\centering
%}{
%\end{minipage}
%}



\input{wortdefinitionen} % Configure wortdefinitionen
\input{pgfplotlistdefinition}
\input{dnadiagram}
\input{lstlistingsupport}

\begin{document}
\newgeometry{left=2cm, right=2cm, top=2cm, bottom=2cm, paper=a4paper}
\AddToShipoutPicture*{\BackgroundPic}
\maketitle
\vspace*{6.2cm}
\hspace{0.3cm}\rotatebox{45}{\textcolor{black!20}{\tiny{Designed by Harryarts}}}\par
\thispagestyle{empty}

%----------------------------------------------------------------------------------------
%	TABLE OF CONTENTS
%----------------------------------------------------------------------------------------

%\setcounter{tocdepth}{1} % Uncomment this line if you don't want subsections listed in the ToC

\restoregeometry
\newpage
\tableofcontents
\newpage

%----------------------------------------------------------------------------------------
%	PROBLEM 1
%----------------------------------------------------------------------------------------

% To have just one problem per page, simply put a \clearpage after each problem

\input{journal}
\cleardoublepage
\phantomsection
\addcontentsline{toc}{section}{Acronyms}
\printacronyms[include-classes=acro]
\cleardoublepage
\phantomsection
\addcontentsline{toc}{section}{Word definitions}
\printacronyms[exclude-classes=acro,name=Word definitions]
\cleardoublepage
\phantomsection
\addcontentsline{toc}{section}{\listfigurename}
\listoffigures
\cleardoublepage
\phantomsection
\addcontentsline{toc}{section}{\lstlistingname}
\lstlistoflistings
\cleardoublepage
\phantomsection
%\addcontentsline{toc}{section}{\bibname}
\bibliographystyle{apacite}
\bibliography{literatur}

%----------------------------------------------------------------------------------------

\end{document}