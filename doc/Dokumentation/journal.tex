...
In our apprenticeship, we both have a lot to do with algorithms. Damian works as a software engineer, where he writes software most of the time. Stefan is an electronics engineer and his specialisation in the fourth year of education is software as well. According to that, we have a pretty good idea of what algorithms are and what they do. We discover, that both of us are fascinated by the concepts and possibilities algorithmic structures provide, especially in practical applications. So we came up with the idea to implement an algorithm by ourselves. \\

%goals
We set the goal to program an algorithm, that is able to play the game \acl{UTTT} (\ac{UTTT}). \ac{UTTT} is a more complex and strategic version of the ordinary Tic-Tac-Toe. 
Our approach to solve that problem is a so called \hyperref[sec:slap]{\acl {SLAP}  algorithm}, also known as \ac{SLAP} Algorithm. It's a combination of the already existing \hyperref[abpalgo]{\acl{ABP} algorithm} (\ac{ABP}), combined with an own made up extension, which is, as the name suggests, \hyperref[selflearning]{self learning}. We want to analyse the learning process of the algorithm in more detail and evaluate whether there is really an improvement. This will be the mathematical part of our work.
To experience our project in action, we also wanted to create a website where everybody can play \ac{UTTT}  against our \ac{SLAP} algorithm.
...
That includes to find a way to value the \acp{game state} in the most efficient way. This will be the task of the \acl{SL} algorithm (\ac{SL}). Only if that part of the \ac{SLAP} algorithm works fine, the \ac{ABP} algorithm is able to work in our favour.
We decided to implement this project with a modern programming language called Dart from Google. Dart is a well-structured, object-oriented programming language that gives us the flexibility to write client and server-side code, all with one code base and one language. Once everything is set up, we will track the development of the \ac{SL} algorithm. We are also planned to integrate a function into the website where the user can evolve his own \ac{SLAP} algorithm and observe the progress themselves.
%overview structure of the paper
%to do

\section{Motivation}
Since the beginning of searching a topic for our IDPA, we knew that we were interested in a practical project rather than a project in written form. Since we earn our money with programming, a software project was the most obvious option. Additionally, creating an own software project from scratch was a desirable idea for both of us.
Damian came up with the idea, to develop a kind of self learning algorithm, which is able to play \ac{UTTT}. We both were really interested in a practical implementation of such an algorithm. We knew we could learn a lot with a project like this and the context of the IDPA provides the perfect opportunity. \\

We knew too, that we could integrate knowledge in our project, which we gained at our mathematics teaching during the last three years. To evaluate and describe certain behaviours of our algorithm, algebra and data analysis provide perfect tools.

To cover two subjects in our IDPA, we decided to apply our English knowledge, which we also improved during the last years at the BMS as well. Because we write all our software in English, we came up with the idea to keep language consistency throughout our whole project, including the website and this documentation.

\subsection{Why we chose Dart}
At the beginning of every programming project there is always the question which language to use. Since we didn't know exactly how we wanted to implement the program, we decided to use a versatile language. Dart runs in the browser, respectively you can compile it in JavaScript, and it runs on the server side. If necessary we could even build the code into an app for Android or IOS. The only, just as versatile language and alternative would be JavaScript. However, Dart was designed to compete with JavaScript.  The main goal was to eliminate the errors and inconveniences of JavaScript. Thus Dart offers an excellent asynchronous language support, which is unfortunately missing in JavaScript. Furthermore the language is easier to read, because it's data type based and has many small features, which make the code more compact. Moreover, both of us have no experience with Dart yet, what we would like to change. And in this project we saw our chance to do that. All these little things let us decide for Dart. The following is a short example of what is meant by the small features that make the code of Dart more compact compared to JavaScript.
...
\section{Rules}
To understand how to play \ac{UTTT}, it's necessary to know the rules of the ordinary Tic-Tac-Toe game. If you know them already, you can continue reading \autoref{rulesuttt}. 
...
\begin{fixedpic}
	...
	\captionof{figure}{A random scenario of an ordinary Tic-Tac-Toe board.}
\end{fixedpic}
...
A player can start anywhere on a local board. According to the location he played on a local board, his opponent will be sent to that relative position in the global board. The opponent can now mark a tile on that local board, keeping in mind that he will send the other player to the relative position on the global board. \\

\begin{fixedpic}
	\centering
	\includegraphics[width=0.6\textwidth]{rule1}
	\captionof{figure}{The yellow player played the last move and forced the blue player to make his turn somewhere in the light blue marked local board. The relative position (top-left) of the yellow square on the small board is the same relative position (top-left) as the light blue marked local board on the global board.}
\end{fixedpic}
...
If a local board is won or draw, no more moves are allowed there. In case a player is sent to such a board, he is allowed to make his next turn on every other free local board. It's possible the game ends in a draw, because no more legal moves are allowed and nobody won.

\begin{fixedpic}
	\centering
	\includegraphics[width=0.6\textwidth]{uttt}
	\captionof{figure}{A random scenario of an Ultimate Tic-Tac-Toe board}
\end{fixedpic}

The yellow marked local fields are already won by the yellow party.

\section{\acs*{SLAP} algorithm} \label{sec:slap}
...
Each game state has a certain number of moves that can be played. For example, when the game starts and the board is empty, there are 81 possible moves which can be played.
All possible moves are being tested one after the other. After each move we get a new game state, which leaves us with possible moves again. Even after these moves, we have new game states again. 
...
Now the tree gets evaluated to get a score as high as possible for us.
We assume that the opponent always makes the move that would be worst for us, because a bad game state for us is a good game state for the opponent. 
...
In our case, for simplification purposes, the \ac{ABP} search tree can be pictured with five branches emerging from every new node. With a search depth of $n$ and without the optimizations the \ac{ABP} algorithm provides, the calculated amount of all nodes, respectively game states, can be represented with the following equation:  $$a = \sum_{i=1}^{n} \delta^{i}$$
...
\end{fixedpic}
\end{itemize}
The following three conditions are checked in the large board.\\
\begin{itemize}
...
After the subtraction we have five certain numbers, which describe each of the five conditions.  Now, we have to convert them into a meaningful number. This is where the DNA comes into play. The DNA is an object consisting of 5 values. Each value stands for a factor by which the number, that represent a  condition, is multiplied. Not all conditions should contribute the same value to the final game state evaluation. With the DNA, we are able to control the relative proportions of the five conditions. Let's do a sample calculation using the picture above. Our example DNA has the following Numbers. 
\begin{center}
\begin{description}
...
\end{lstlisting}
Since these functions are needed for each individual move, we made these functions as efficient as possible. So we already evaluated every possible Tic-Tac-Toe board and stored it in a cache. If we now evaluate the \ac{UTTT} board, we can access this information and minimize the computing time per call.
...
Before we could start with the \ac{ABP} algorithm, we had to implement the heuristics. The heuristic is used by the Alpha Beta Pruning algorithm. For the heuristic we first created the DNA, the basic component of the game state evaluation. Based on this DNA, the five occurring conditions on a board  are then added together and describe a game state with a meaningful number. In order to accelerate the game state evaluation, we already loaded the evaluation for all possible Tic-Tac-Toe boards in the background, as with the game controller. With this information we were able to efficiently evaluate even a large board without much computational effort. This optimization is needed because this function is called by the \ac{ABP} algorithm for every possible game state.
...
\section{Challenges}
Errors always creep in during programming. Some are quickly noticed, others remain unnoticed. Of course we had some issues with bugs and unwanted behaviour, but only two cases really cost us time.

\subsection{Worker}
A worker is a feature of web development. You can outsource computational tasks to them so that the main thread, which is responsible for updating the UI, is not under load for to long. Otherwise the surface is no longer drawn cleanly and the use of the web application no longer feels fluid. Creating such a worker would not have been a problem with JavaScript, but Dart had his pitfalls. Since the programming language is still in development, this feature was not yet implemented cleanly. But we still wanted to use it in order to provide a better user experience.\\
To create a worker, we had to link a JavaScript file, which gets executed in another thread. Our plan was to call a JavaScript file, which afterwards would execute a Dart file, compiled in JavaScript. The problem we faced: There is no official way, how to call a Dart file out of a JavaScript file. We had to look deep in the documentation of the Dart compilers in order to find relevant informations to solve this problems. In the end, the worker worked, even tough it puzzled us many hours.
...
To meet this requirement, fortunately we had the Materialize CSS framework. It provided us useful tools to design the website for different screen sizes. Despite that, it wasn't always easy to achieve a seamless design on different end-devices. While developing the website, some components were often misplaced, when we accessed the unfinished website with different end-devices.
Although it was challenging, we had a well structured responsive design on most devices.

%\todo{review allenfalls ausführlicher, Edit Damian: Habe noch einzwei sätze hinzugefügt.}
\section{Review}
In retrospect, the project was very interessting. For us, it was rewarding to see the project grow while achieving small goals. Despite the fact that the algorithm works in theory, we had our concerns it wouldn't work seamlessly in a practical implementation. It was a relieve, when we saw  the self learning part of the algorithm evolved in accordance to our expectations. Through this project we could learn a lot. Not only was the programming language new, but also hosting a website and implementing a self-learning algorithm which plays a game. Both of us haven't done that yet.
...
Our algorithm is far from perfect. One point that could be improved is the structure of the DNA. Actually, the weighting of a condition is not that important. More important is the difference to the other parameters. Thus, the DNA could have been reduced to four parameters, which would have made training an era faster. Either by giving the first parameter a fixed number instead of mutating, or by describing DNA parameters not as a condition but as a difference to the previous parameter.
