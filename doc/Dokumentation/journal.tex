...
That includes to find a way to value the states of the game in the most efficient way. This will be the task of the \acl{SL} algorithm (\ac{SL}). Only if that part of the \ac{SLAP} algorithm works fine, the \ac{ABP} is able to work in our favour.
...
\section{Rules}
To understand how to play \ac{UTTT}, it is required to  know the rules of the ordinary Tic-Tac-Toe game. If you already know the rules of the ordinaty Tic-Tac-Toe, you can continue reading paragraph 2.2. 
...
\todo{minimale oder maximaler durchschnitt. der genaue durchschnitt ist unbekannt}

The average amount of options per move in the game \ac{UTTT} is five. \cite{web:tsurel2013}
...
\end{fixedpic}
\item Two fields occupied, one field empty. Here the condition occurs in the lowest row. Note how the first condition occurs three times again, in the left column vertically, diagonally left bottom to the right top and again vertically middle column.\\
\begin{fixedpic}
...
\todo{akronyme momentaner standpunkt}
...
As we already found out in \autoref{Self Learning}, on a computer with 16 threads we needed an average of $[t] = 1,714s$. If we multiply that by the 14 ($maximale\ number\ threads - 2$) working threads, we get $[t] = 24 s$ per game per thread.
...
\todo{Animista und Materialize verlinken}
...
\end{fixedpic}
\todo{evtl Figure 16 kommentar bearbeiten}
...
\todo{review allenfalls ausführlicher}
\section{Review}
